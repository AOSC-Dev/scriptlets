...
sub version_compare_mini($$) {
    my ($a, $b) = @_;
    my $pattern = qr/(?:(?<epoch>\d+):)?(?<version>[^-\n]+)(?:-(?<rel>\d+))?/msp;
    my %a_version = %+ if ($a =~ /$pattern/);
    my %b_version = %+ if ($b =~ /$pattern/);
    return unless (%a_version and %b_version);
    my $epoch_result = int($a_version{'epoch'}) <=> int($b_version{'epoch'});
    return $epoch_result unless $epoch_result == 0;
    my $version_result = 0;
    if ($a_version{'version'} =~ /[^0-9\.]+/ or $b_version{'version'} =~ /[^0-9\.]+/) {
        $version_result = $a_version{'version'} cmp $b_version{'version'};
    } else {
        $version_result = version->parse(sprintf("v%s", $a_version{'version'})) <=> version->parse(sprintf("v%s", $b_version{'version'}));
    }
...
sub fetch_packages($$@) {
    ...
    for my $dep (@packages) {
        ...
        my $filename = download_file(url => "$mirror/$pkg{'filename'}", to => $cache_dir);
        print "[$count/$length] Verifying $pkg{'name'}...\n";
        digest_file_hex("$cache_dir/$filename", "SHA-256") == $pkg{'sha'} or die "Failed to verify $pkg{'name'}";
...
sub chroot_do($@) {
    my $target = shift;
    my @cmd = @_;
    system('chroot', "$target", @cmd) == 0 or die("Command failed when executing in chroot: $cmd[0]\n");
}

sub chroot_script_do($$) {
    my $target = shift;
    my $script = shift;
    my ($fh, $filename) = tempfile("ab-XXXXXX", SUFFIX => '.sh', DIR => "$target/aoscbootstrap") or die("Cannot create temporary file");
    print $fh "$script\n";
    close($fh);
...
sub bootstrap_apt(%) {
    ...
    my $branch = $args{'branch'};
    my $target = $args{'target'};
    make_path("$target/var/lib/dpkg") or die "Could not create directory" unless -d "$target/var/lib/dpkg";
    make_path("$target/etc/apt") or die "Could not create directory" unless -d "$target/etc/apt";
    die "Unable to bootstrap Dpkg available file." unless open(FH, ">$target/var/lib/dpkg/available");
    close(FH);
    die "Unable to bootstrap Dpkg state file." unless open(FH, ">$target/var/lib/dpkg/status");
    close(FH);
    die "Unable to bootstrap APT sources file." unless open(FH, ">$target/etc/apt/sources.list");
    print FH "deb $mirror $branch main\n";
    close(FH);
    die "Unable to bootstrap locale file." unless open(FH, ">$target/etc/locale.conf");
    print FH "LANG=C.UTF-8\n";
    close(FH);
    die "Unable to bootstrap shadow file." unless open(FH, ">$target/etc/shadow");
...
sub mknod(@) {
    ...
    my $minor=shift;
    return if -e "$device";
    system(("mknod", "-m", "666", $device, $type, $major, $minor)) == 0 or die "Could not create device $device";
}
...
sub bootstrap_dev_nodes($) {
    ...
    make_path("$target/dev/shm") or die "Failed to mkdir $target/dev/shm" unless -d "$target/dev/shm";
...
sub generate_dpkg_install_script(@) {
    my (@packages) = @_;
    my $script = '';
    open(my $fh, '>', \$script) or die "Could not open an in-memory file descriptor...\n";
    print $fh "#!/bin/bash\ncount=0\nPACKAGES=(\n";
    ...
    print $fh "  dpkg --force-depends --unpack \"/var/cache/apt/archives/\${p}\"\ndone\n";
    print $fh "dpkg --configure --pending --force-configure-any --force-depends\n";
    close($fh);
    return $script;
}
...
my %args_3 = ('manifests' => \@manifests, 'packages' => \@stub_packages, 'target' => $target);
